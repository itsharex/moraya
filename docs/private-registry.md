...
`@zouwei/moraya-core` is a **private** npm package hosted on GitHub Packages. Installing it requires a GitHub Personal Access Token (PAT) with the `read:packages` scope. This document covers the auth flow for local development, GitHub Actions CI, and Docker builds.
...
2. Token name: `moraya-core read` (or similar)
3. Expiration: 90 days (renew as needed)
4. Repository access: **Only select repositories** → `zouwei/moraya-core`
5. Permissions:
...
```bash
pnpm install   # should resolve @zouwei/moraya-core without 401
pnpm tauri dev
```
...
**Cross-org consumption** (e.g. moraya-web is in a different org from moraya-core): the workflow needs a fine-grained PAT stored as a repo secret (e.g. `GH_PACKAGES_READ_TOKEN`), used in place of `secrets.GITHUB_TOKEN`.
...
# RUN echo "@zouwei:registry=https://npm.pkg.github.com" > .npmrc \
...
Until `@zouwei/moraya-core@0.1.0` is officially published to GitHub Packages, this repo consumes a **vendored tarball** under `./vendor/moraya-core-0.1.0.tgz`:

```jsonc
// package.json
{
  "dependencies": {
    "@zouwei/moraya-core": "file:vendor/moraya-core-0.1.0.tgz"
  }
}
```
...
| `npm error 403 Forbidden` | PAT cannot access this specific repo | Make the PAT fine-grained and add `zouwei/moraya-core` to "Selected repositories" |
| `npm error 404 Not Found` for `@zouwei/moraya-core` | Registry URL incorrect or token has no read access | Verify `@zouwei:registry=https://npm.pkg.github.com` in `~/.npmrc` |
| `ERR_PNPM_OUTDATED_LOCKFILE` after `package.json` swap to `^0.1.0` | Lockfile still references the tarball | Delete `pnpm-lock.yaml` entry for `@zouwei/moraya-core` and re-run `pnpm install` |
| Boundary CI gate ("Forbid sibling-path imports") fails | Some PR introduced `from '../moraya-core/...'` or a `link:` / `workspace:` declaration | Replace with the published package import; see spec §1.3 |

---

## Boundary reminder (spec §1.3 — hard rule)

`@zouwei/moraya-core` is consumed via the published package only. Any of the following blocks PR merge via [.github/workflows/ci.yml](../.github/workflows/ci.yml) "Forbid sibling-path imports of @zouwei/moraya-core":

- ❌ `import { ... } from '../moraya-core/...'`
- ❌ `import { ... } from './moraya-core/...'`
...
The `./moraya-core/` symlink at the repo root is for cross-repo editing convenience only and is gitignored. Removing it must not affect `pnpm install && pnpm tauri build`.
